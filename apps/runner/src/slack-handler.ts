...
/** Fetches a Slack private file as an ArrayBuffer using the bot token. */
...
/**
 * Downloads a Slack file and returns a Claude ContentBlockParam:
 * - text/md → TextBlockParam (inline in prompt text via caller)
 * - image   → ImageBlockParam (base64)
 * - pdf     → DocumentBlockParam (base64)
 * - other   → null (skip)
 *
 * Returns `{ kind: 'text', content: string }` for text files so the caller
 * can inline them in the prompt string rather than as a separate block.
 */
...
async function buildPrompt(
  ...
  // If there are binary blocks, return a multimodal content array
