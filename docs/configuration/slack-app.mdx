...
- The following **Redirect URL** added under **OAuth & Permissions → Redirect URLs**: `https://your-domain/api/auth/slack/callback`
- The `openid`, `email`, and `profile` scopes added under **OAuth & Permissions → User Token Scopes**

Once you have these, go to **Settings → General → Authentication** in SlackHive and enter the Client ID and Client Secret.
